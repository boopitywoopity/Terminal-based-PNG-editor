...
void load_colours_select_fill(int init_y,int init_x,int range_y,int range_x,int width,int height);
...
/* init_color(pair_index,r*1000/255,g*1000/255,b*1000/255); */
/* init_pair(pair_index,pair_index,COLOR_BLACK); */
...
int main(int e, char **args){
        ...
        // organise the colours in a well organised way
        user_colours = malloc(sizeof(struct point_w_color)*10);
        ...
        colours = malloc(sizeof(struct RGB_spectrum)*MAX_COLOURS); 
        total_image = malloc(sizeof(struct point_w_color)*height*width); 
        if (total_image == NULL){
                endwin();
        ...
        for (int y=0;y<height;y++){
                total_image[y] = malloc(sizeof(struct point_w_color*) * 400000); // alocates the memory per line
                for (int x=0;x<width;x++){
                        ...
                        if (image[index+3]!=0){ // this makes sure the alpha (transparency) is not set to 0 which would make the pixel invisible
                                struct RGB_spectrum colour;
                                ...
                                if (colour_position != -1){
                                        ...
                                        if (current_colours+1 == MAX_COLOURS){
                                                endwin();
...
void key_press(unsigned int *x,unsigned int *y,int *width,int *height,char input,int *quit){
        ...
        switch(input){
                ...
                case ':': // handles commands from the user
                        ...
                        if (true); // this is there because if it isnt the lsp screams at me that there is an error, but it still runs...
                        int command_len = 0;
                        ...
                        if (compare_at(command,"q",1)){
                                *quit = 1;
                                ...
                                for (int i=0;i<10;i++){ // generates the colors for the user to view the colour templates
                                        float colour_brightness = (user_colours[i].r * 0.299 + 0.587*user_colours[i].g + 0.144*user_colours[i].b)/255;
                                        init_color(17+i,user_colours[i].r*1000/255,user_colours[i].g*1000/255,user_colours[i].b*1000/255);
                                        if (colour_brightness < 0.5){
                                                init_pair(17+i,COLOR_WHITE,17+i);
                                        }
                                        else{
                                ...
                                int col_cmd_len = 0;
                                char col_input;
                                int cmd_selected_color = -1; // this is the selected color that the user has chosen for the action
                                        ...
                                        if (strcmp(colour_command,"-use") == 0){
                                                ...
                                                while (!accept){
                                                        ...
                                                        if (cur!=2){
                                                                ...
                                                                float colour_brightness = (r * 0.299 + 0.587*g + 0.144*b)/255;
                                                                init_color(16,r*1000/255,g*1000/255,b*1000/255);
                                                                if (colour_brightness < 0.5){
                                                                        init_pair(16,COLOR_WHITE,16);
                                                                }
                                                                else {
                                                ...
                                                move(LINES-1,0);
                                                clrtoeol();
                                                printw("Invalid input the command you gave in was %d%s",cmd_selected_color,colour_command);
                                                getch();
                                        }
                                }
                                free(colour_command);
        ...
        for (int y=0;y<height;y+=pixel_ratio){
                ...
                for (int x=0;x<width;x+=pixel_ratio){
                        ...
                        if (a_total != 0){
                                ...
                                int colour_position = colours_contains(colour);
                                if (colour_position != -1){
                                        colours[colour_position].point[colours[colour_position].length].x = x_pos;
                                        colours[colour_position].point[colours[colour_position].length].y = y_pos;
...
void select_from_image(unsigned int *x,unsigned int *y,int width, int height){
        struct xy_point *coord = malloc(sizeof(struct xy_point)*2000);
...
void load_colours_select(struct xy_point *points,int point_num,int width,int height){
        ...
        for (int i=0;i<current_colours;i++){
                ...
                init_color(colour_index,colours[i].r*1000/255,colours[i].g*1000/255,colours[i].b*1000/255);
                init_pair(colour_index,COLOR_BLACK,colour_index);
...
void load_colours_select_fill(int init_y,int init_x,int range_y,int range_x,int width,int height){
        ...
        for (int i=0;i<current_colours;i++){
                ...
                for (int n=0;n<colours[i].length;n++){
                        int cur_y = colours[i].point[n].y;
                        int cur_x = colours[i].point[n].x;
                        if ((cur_y >= y_buffer && cur_y < LINES-2 +y_buffer) && (cur_x >= x_buffer && cur_x < COLS-1 + x_buffer) && cur_x-x_buffer<COLS-1 && cur_y-y_buffer < LINES-2)
                        {
                                if (in_range(init_y,y_range,cur_y) && in_range(init_x,x_range,cur_x)){
                                        mvaddch(cur_y - y_buffer, cur_x - x_buffer, '0');
                                }
                                else{
                                        mvaddch(cur_y-y_buffer,cur_x-x_buffer,' ');
...
int load_colours(int width,int height){
        ...
        for (int i=0;i<current_colours;i++){
                ...
                for (int n=0;n<colours[i].length;n++){
                        int cur_y = colours[i].point[n].y;
                        int cur_x = colours[i].point[n].x;
                        if ((cur_y >= y_buffer && cur_y-y_buffer < LINES-2) && (cur_x >= x_buffer && cur_x-x_buffer < COLS-1))
                                mvaddch(cur_y-y_buffer,cur_x-x_buffer,' ');   
...
void load_image_information(int x,int y,int height,int width){
        ...
        mvprintw(LINES-2,COLS-20,"(%d,%d)     %d",y+y_buffer,x+x_buffer,loop_count);
